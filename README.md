...
### ✦ 이탈(휴면) 정의(라벨)
- **m2(휴면)**: *(예시)* Anchor 시점(t) 이후 **H=30일 동안 접속/장바구니/구매 이벤트가 0회**이면 휴면으로 정의. 
...
본 프로젝트는 **휴면상위 K% 고객에게만 쿠폰/케어를 수행**하는 운영 목표를 가정합니다.
...
> Note : 본 프로젝트는 **Top-K 운영**이므로, 모델 선택은 **Lift@K/Precision@K를 우선**하고 PR-AUC는 보조로 사용한다.
