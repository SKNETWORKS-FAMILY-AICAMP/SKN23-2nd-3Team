...
- 
- 사용 이유 : 가장 단순하고 빠르며,  class_weight="balanced"와 StandardScaler 기반으로 불균형 이진 분류에서 해석 가능하고 안정적인 기준선을 확보하고, 이후 모델 성능 향상을 비교할 공정한 베이스라인으로 사용했습니다.
...
- 
- 사용 이유 : 로그 기반 탭 피처에서 비선형 관계·특성 상호작용을 강하게 학습하는 대표적인 트리 부스팅 모델로, 불균형 데이터에서도 class_weight="balanced" + early stopping으로 안정적으로 성능을 끌어올리기 위해 사용했습니다.
...
sample_weight를 직접 구성해(make_sample_weight) 불균형을 보정하며, 비교적 단순한 파이프라인으로도 강한 성능을 기대할 수 있어 채택했습니다.
...
## MLP(Base)
- 개념 : 표준화된 입력 피처를 다층 신경망(MLP)으로 학습해, 선형/트리 모델이 잡기 어려운 연속적 비선형 패턴을 근사하는 딥러닝 기반 분류기.
  
- 사용 이유 : ML 모델(LR/Boosting) 대비 더 유연한 함수 근사를 통해 복잡한 상호작용을 학습할 수 있어 DL 라인을 구성했으며, base 버전은 구조를 단순하게 유지해 확장(enhanced/advanced) 대비 성능 개선의 출발점으로 사용했다. 또한 불균형을 pos_weight로 보정하여 m2(positive)를 학습에서 놓치지 않도록 했다.

- Top-K 타겟팅과의 연결: 튜닝 기준을 **val Recall@TopK 우선(동률이면 PR-AUC)**으로 설정해, 제한된 예산에서 상위 K% 타겟팅 시나리오에 맞춰 “휴면자를 Top-K에서 최대한 포착”하도록 모델을 선택했다
- 필요 자료 -> 모델 평가지표 비교 그래프 및 표 / 대표 코드
 
## MLP(Enhence) 
  - 개념 : 트리 기반(Boosting)과 다른 패밀리(DL)로서, 선형/트리 모델이 놓칠 수 있는 연속적 비선형 관계를 학습하기 위해 MLP를 사용했다. 특히 Enhanced는 base 대비 구조를 확장해 성능 상향 여지를 검증하기 위한 단계  
- Top-K 타겟팅과의 연결 : 우리의 운영 목표는 “상위 K%만 쿠폰 지급”이므로, 하이퍼파라미터 선택 기준을 **val Recall@TopK 우선(동률이면 PR-AUC)**으로 두었다. 즉 확률 점수를 잘 “랭킹”해서 상위 K% 내에 휴면(m2)을 최대한 많이 포함시키는 방향으로 튜닝

  - **표준화된 탭불러 피처를 입력으로 받는 다층 퍼셉트론(MLP) 기반 이진 분류 모델로, 비선형 상호작용을 학습해 휴면(m2) 위험도를 확률 점수로 산출한다**

- Top-K 타겟팅과의 연결 : Top-K 타겟팅과의 연결 : 우리의 운영 목표는 “상위 K%만 쿠폰 지급”이므로, 하이퍼파라미터 선택 기준을 **val Recall@TopK 우선(동률이면 PR-AUC)**으로 두었다. 즉 확률 점수를 잘 “랭킹”해서 상위 K% 내에 휴면(m2)을 최대한 많이 포함시키는 방향으로 튜닝  
  
- Top-K 타겟팅과의 연결 : 운영은 “상위 K%만 쿠폰 지급”이므로, best 모델 선택 기준을 **val Recall@Top10% 최대화(동률이면 PR-AUC)**로 설정했다. 즉 0.5 임계값 분류가 아니라, 랭킹에서 상위 K%에 휴면(m2)을 최대한 포함시키는 방향으로 학습을 선택
